...
import DeleteModal from "@/app/admin/agenda/delete/page";
...
import {
	...
	TABLE_QUERY,
	type OrderQuery,
} from "./generic-table.enums";
import { Dialog, DialogContent, DialogTrigger } from "../ui/app-dialog";

export function GenericTableRoot({
	children,
	className,
	...props
...
export function GenericTableContent<TItem extends { id: string }>({
	...
	const currentOrderQuery =
		(getQueryString(TABLE_QUERY.ORDER) as OrderQuery) ?? DEFAULT_ORDER;

	const hiddenQuery = (getQueryString(TABLE_QUERY.HIDDEN)?.split(",") ??
		[]) as (keyof TItem)[];
