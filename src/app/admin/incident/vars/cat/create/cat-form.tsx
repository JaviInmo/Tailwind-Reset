...
export function CatForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setGlobalError("Error loading variables. Please try again.")
      }
    }
  ...
  const handleVariableChange = (value: string) => {
    console.log("Selected variable value:", value)

    // Try to find by name first
    let selectedVariable = variables.find((v) => v.name === value)

    // If not found by name, try to find by ID (in case value is an ID string)
    if (!selectedVariable && !isNaN(Number(value))) {
      selectedVariable = variables.find((v) => v.id === Number(value))
    }

    if (selectedVariable) {
      console.log("Found variable:", selectedVariable)
      setSelectedVariableId(selectedVariable.id)
    } else {
      console.log("Variable not found in:", variables)
    }
  }
  ...
  const onSubmit: SubmitHandler<FormSchemaData> = async (data) => {
    ...
    if (!selectedVariableId) {
      console.error("No variable ID selected")
    ...
    for (const category of categoriesArray) {
      console.log("Creating category:", category, "for variable ID:", selectedVariableId)
  ...
  const handleCloseSuccessModal = () => {
    ...
    // Go back to the listing page and refresh
    router.back()
    router.refresh()
  }
      ...
      <div className="w-full space-y-7 rounded-sm bg-white py-8">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500">
            Seleccione una variable y rellene el campo del formulario para registrar las categorías.
          </p>
        </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="w-full max-w-lg space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Nombre de las Categorías (separadas por comas):</FormLabel>
                    <FormControl>
                      <Input placeholder="Introduzca las categorías separadas por comas" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
