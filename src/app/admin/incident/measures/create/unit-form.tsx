...
import { createUnitAction } from "./unit.action"
...
export function UnitForm() {
  ...
  const onSubmit = async (data: FormSchemaData) => {
    ...
    try {
      const response = await createUnitAction(data)

      if (!response.success) {
        setGlobalError(response.error || "Error al crear la unidad de medida")
        return
      }
      ...
      <div className="flex h-full w-full flex-col items-center justify-center rounded bg-white py-2 shadow-sm text-black">
        <div className="w-full max-w-md">
          <div className="mb-4 text-center">
            <h1 className="mb-3 text-2xl font-semibold">Registro de Unidad de Medida</h1>
            <p className="text-black">Ingrese el nombre de la unidad de medida.</p>
          </div>
