...
export default function UsersTable({ data, pageCount, currentPage }: TableProps) {
  ...
  // Estado para mantener los IDs de los usuarios que se muestran
  const [userIds, setUserIds] = useState<string[]>(data.map((item) => item.id));
  ...
  const confirmDelete = async (id: string) => {
    const result = await handleDeleteUserAction(id, userIds.join(","));
    if (result.success) {
      ...
      <div className="relative max-h-[calc(100vh-200px)] w-full overflow-x-auto overflow-y-auto rounded-lg border border-slate-300">
        <Table className="w-full">
          ...
          <TableBody className="text-slate-700">
            {data
              .filter((row) => userIds.includes(row.id))
              .map((row, rowIndex) => (
                <TableRow
                  key={row.id}
                  ...
                  {columns.map(
                    ({ key }) =>
                      visibleColumns[key] && (
                        ...
                        title={key === "id" ? String(rowIndex + 1) : String(row[key])}
                      >
