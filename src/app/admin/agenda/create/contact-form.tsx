...
// Esquema del formulario utilizando zod (nombre de campos para la UI)
...
// Tipo que espera la server action (nombres usados en el backend)
type ServerFormData = {
  name: string;
  provinceId: string;
...
// Tipos para provincias/municipios
type Municipality = {
  id: string;
  name: string;
};
...
type ContactFormProps = {
  ...
  readOnly?: boolean;
  // customSubmit debe aceptar el shape del servidor (provinceId / municipalityId)
  customSubmit: (data: ServerFormData) => Promise<{ success: boolean; contact?: any; error?: string }>;
};

export function ContactForm({ contactData, provinceData, readOnly, customSubmit }: ContactFormProps) {
  ...
  async function onSubmit(data: FormSchemaData) {
    ...
    // Mapear nombres del formulario -> nombres que espera el backend
    ...
    try {
      const response = await customSubmit(payload);

      if (!response?.success) {
        const msg = response?.error ?? "Contacto ya registrado / error del servidor";
        return setError("root", { message: msg });
      }

      reset();
      setShowSuccessModal(true);
    } catch (err: any) {
      setError("root", { message: err?.message ?? "Error inesperado al enviar" });
    }
  }
      ...
      <div className="w-full flex flex-col items-center justify-center rounded bg-white py-2 shadow-sm">
        <div className="flex flex-col items-center justify-center w-full !lg:min-h-[calc(100vh-75px)]">
          <div className="pb-4 text-center">
            ...
            <p className="font-semibold">Rellene los campos del formulario para registrar el contacto.</p>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="flex h-full w-full flex-col gap-4 overflow-y-auto px-4 lg:w-11/12">
            <div className="flex gap-4 pb-2">
              <div className="w-full">
                <Label className="block pb-2">Nombre Completo:</Label>
                <Input type="text" {...register("name")} className="w-full rounded border border-gray-300 bg-white p-2" />
                {errors.name && <p className="text-red-600">{errors.name.message}</p>}
              </div>
              <div className="w-full">
                <Label className="block pb-2">Teléfono Fijo:</Label>
                <Input type="text" {...register("landlinePhone")} className="w-full rounded border border-gray-300 bg-white p-2" />
                {errors.landlinePhone && <p className="text-red-600">{errors.landlinePhone.message}</p>}
              </div>
            </div>
            ...
            <div className="flex gap-4 pb-2">
              ...
              <div className="w-full">
                ...
                <Controller
                  ...
                  name="municipality"
                  render={({ field: { onChange, value } }) => (
                    <Select onValueChange={onChange} value={value} disabled={!provinceId}>
            ...
            <div className="flex gap-4 pb-2">
              <div className="w-full">
                ...
                <Input type="text" {...register("personalPhone")} className="w-full rounded border border-gray-300 bg-white p-2" />
                {errors.personalPhone && <p className="text-red-600">{errors.personalPhone.message}</p>}
              </div>
              <div className="w-full">
                <Label className="block pb-2">Teléfono Estatal:</Label>
                <Input type="text" {...register("statePhone")} className="w-full rounded border border-gray-300 bg-white p-2" />
                {errors.statePhone && <p className="text-red-600">{errors.statePhone.message}</p>}
              </div>
            </div>
            ...
            <div className="flex gap-4 pb-2">
              <div className="w-full">
                ...
                <Input type="text" {...register("jobTitle")} className="w-full rounded border border-gray-300 bg-white p-2" />
                {errors.jobTitle && <p className="text-red-600">{errors.jobTitle.message}</p>}
              </div>
              <div className="w-full">
                <Label className="block pb-2">Lugar de Trabajo:</Label>
                <Input type="text" {...register("workplace")} className="w-full rounded border border-gray-300 bg-white p-2" />
                {errors.workplace && <p className="text-red-600">{errors.workplace.message}</p>}
              </div>
            </div>
            ...
            <div className="flex justify-center">
              <Button type="submit" className="w-2/4 rounded border-slate-700 bg-slate-800 py-2 text-slate-100 hover:bg-slate-950">
                Crear Contacto
              </Button>
            </div>
          </form>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="mx-auto w-11/12 max-w-md rounded-lg bg-white p-4 shadow-lg">
              ...
              <div className="flex justify-end pt-2">
                <Button onClick={handleCloseSuccessModal} className="rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-600">
                  Aceptar
                </Button>
              </div>
            </div>
